...
chaincodeInstantiate () {
	...
	if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
		peer chaincode instantiate \
			...
			-n $CC_NAME \
			-v 1.0 \
			-c '{"Args":["ebb2c6d8ec37a50b1f755da14d3874e8fd3510678665da27391f063387ad4c03"]}' \
...
chaincodeInstall () {
	...
	VERSION=$2
	setGlobals $PEER
	peer chaincode install -n $CC_NAME -v $VERSION -p github.com/hyperledger/fabric/examples/chaincode/go/flowchain01 >&log.txt
	res=$?
	cat log.txt
